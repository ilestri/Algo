<template>
  <form class="space-y-2" @submit.prevent="emit('submit', { keys: keys.value, search: search?.value, remove: remove?.value })">
    ...
    <input v-model="keysText" class="w-full rounded border p-2 text-black" />
    <div class="grid grid-cols-2 gap-2">
      <div>
        <label class="block text-sm">검색 키</label>
        <input v-model.number="search" type="number" class="w-full rounded border p-2 text-black" />
      </div>
      <div>
        <label class="block text-sm">삭제 키</label>
        <input v-model.number="remove" type="number" class="w-full rounded border p-2 text-black" />
      </div>
    </div>
